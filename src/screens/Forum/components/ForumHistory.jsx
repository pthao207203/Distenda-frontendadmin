...
const getActionStyle = (action) => {
  switch (action) {
    case "review":
      return { text: "ĐÃ DUYỆT", color: "text-[#34A853]" };
    case "delete":
      return { text: "ĐÃ XÓA", color: "text-[#DF322B]" };
    default:
      return { text: "KHÁC", color: "text-gray-500" };
  }
};

export default function ForumHistory({ onClose }) {
  ...
  useEffect(() => {
    async function fetchData() {
      const result = await courseHistoryController();
      if (result) {
      ...
      {filteredData && filteredData.length > 0 ? (
        <div className="flex flex-col gap-3 justify-start self-start w-full mt-4 max-h-[70vh] overflow-y-auto">
          {filteredData.map((history, index) => {
            ...
            const postTitle = history.postTitle;
