...
export default function ChapterDetailHistory({ onClose }) {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        setLoading(true);
        const result = await videoHistoryController(setLoading, LessonID);
        // console.log(result)
        if (result) {
  ...
  console.log("Video History => ", data);
  ...
  const filteredData = data?.filter((item) => {
    ...
    return (
      ...
      item.VideoName?.toLowerCase().includes(keyword) ||
      ...
      {filteredData && filteredData.length > 0 ? (
        <div className="flex flex-col gap-3 justify-start self-start w-full mt-4 max-h-[70vh] overflow-y-auto">
          {data.map((history, index) => {
            ...
            const videoName = history.VideoName;
